### 🧠 FavourPro - AI 内心状态模拟插件（重构版）

**当前作者**: 柯尔  
**版本**: v1.1.0  
**原作者**: 天各一方

> 📦 **原项目地址**: [Catfish872/astrbot_plugin_favourpro](https://github.com/Catfish872/astrbot_plugin_favourpro)

这是一个与现有好感度系统完全不同的插件。它不再是一个简单的数值计算工具，而是旨在为 AI 模拟一个**多维度、自主且保密的内心世界**。AI 会根据与用户的互动，真实地改变其内在的“好感度”、“态度”和“关系”，并通过回应风格的变化自然地流露出来。

> ⚠️ **重构说明**  
> 本仓库基于原作者天各一方的插件进行重构，是在原版基础上长期自用、持续迭代的魔改版本。新增好感度变化幅度限制功能，优化提示词配置。所有改动保持向下兼容。
...
1.  **核心配置**:
    核心配置仍然兼容原版，但魔改版新增了更细粒度的可调选项，均可在 `config.json` 中设置：
    -   `session_based` (bool): 控制是否为每个会话分别维护状态。
    -   `initial_favour` / `initial_attitude` / `initial_relationship`: 自定义新用户的初始内心状态。
    -   `min_favour` / `max_favour`: 设定好感度的全局上下限，AI与管理员的所有操作都会被限制在该区间。
    -   `instruction_prompt`: 完整自定义 AI 的内心指令，方便根据场景灵活调整。
    -   `admin_permission_denied_msg`: 自定义管理员权限不足时的提示语。
...
#### 🚀 v1.1.0（重构版）
-   🔄 **代码重构**: 基于原作者天各一方的插件进行全面重构，精简冗余代码约30行。
-   ✨ **好感度变化幅度限制**: 新增 `max_increase` / `max_decrease` 配置项，限制AI单次回复中好感度的最大变化幅度，防止异常跳变。
-   🧹 **移除硬编码**: 删除代码中冗余的默认提示词硬编码，统一由配置文件管理。
-   📝 **精简提示词**: 优化默认instruction_prompt，减少约40% token消耗。
-   🛠️ **日志配置简化**: 移除重复的handler配置，使用AstrBot全局日志系统。

#### 🚀 v1.0.6
-   🛡️ **全局拦截器 (Monkey Patching)**: 解决第三方插件（如 Conversa 主动回复）绕过框架钩子导致好感度标签泄露的问题。
    -   **Provider.text_chat 拦截**: 在 LLM 响应返回时自动清理好感度标签，无论调用方是谁。
    -   **Context.send_message 拦截**: 在消息发送前兜底清理，双重保险确保标签不泄露。
    -   **零侵入设计**: 不需要修改任何第三方插件代码，全局生效。
    -   **生命周期管理**: 插件卸载时自动恢复原始方法，不影响框架稳定性。

#### 🚀 魔改增补（基于 v1.0.4）
-   ✨ **新增可配置项**: 支持自定义初始状态、好感度上下限、管理员提示语及完整的内心指令文本。
-   ✨ **AI指令优化**: 默认指令更强调正向激励、幻觉防控与安全策略，可按需覆盖。
-   ✨ **好感度范围限制**: AI 与管理员修改好感度时会自动限制在配置区间内，避免异常数值。
-   🛠️ **解析鲁棒性提升**: 
    -   正则表达式支持中英文标点，更准确提取状态字段。
    -   自动清除一条回复中的全部状态块，杜绝"隐藏消息露出"。
-   🧰 **代码健壮性增强**: 
    -   默认状态改为实例级存储，避免热重载或多实例场景下的交叉污染。
    -   管理指令与提示语采用统一、可配置的输出。
    -   好感度数值解析增加异常保护。
